...
class UsersController < ApplicationController
  ...
  def index
    ...
    when User::ROLE_LEADER
      @users = User.employees_and_volunteers.paginate :page => params[:page]
    when User::ROLE_EMPLOYEE
      @users = User.volunteers.paginate :page => params[:page]
    end
  end
  ...
  def update_password
    ...
    elsif params[:new_password] !~ /^[a-zA-Z0-9_]{4,16}$/
      flash[:alert] = '新密码只能由4-16位的大小写英文字母、数字和下划线组成，请重试'
    else
